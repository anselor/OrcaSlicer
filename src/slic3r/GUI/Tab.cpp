...
void Tab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    if (opt_key == "extruders_count" && !m_config->opt_bool("single_extruder_multi_material")){
        ...
        // Mapped printers (slicer- or device-owned) own their filament count; growing tools never deletes filaments.
        if (physical_filament_features_enabled(*m_config) && num_extruder <= (size_t) old_filament_size) {
            wxGetApp().get_tab(Preset::TYPE_PRINT)->update();
        } else {
