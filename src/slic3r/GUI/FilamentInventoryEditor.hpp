...
private:
    ...
    // them: main_card always shows rows[0]; swap_cards is index-aligned with rows[1..] and is
    // torn down/rebuilt whenever a row is added or removed (rebuild_tool_rows).
    struct ToolGroup
    {
        std::vector<Row> rows;
        FilamentCard*     main_card{nullptr};
        wxBoxSizer*       swap_sizer{nullptr}; // vertical stack of swap-slot cards under main_card
        std::vector<FilamentCard*> swap_cards; // index-aligned with rows[1..]
    };
    ...
    // rebuilds their widgets. Shared by the ctor and add_filament/remove_filament's rebuild path
    ...
    // Printer opt-in for recording manually-swapped filaments; when
    // false the editor offers only the fixed 1:1 loadout (no "Add filament" affordance at all).
    bool                      m_allow_manual_swaps{false};
