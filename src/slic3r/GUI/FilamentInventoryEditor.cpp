...
// swappable filaments (slot 1+, "small" size, with a Remove icon in addition to the pencil).
...
{
public:
    // Tool and swap cards share this width so the column's cards line up; swap cards only differ
    ...
    FilamentCard(wxWindow* parent, bool compact)
        : wxPanel(parent, wxID_ANY, wxDefaultPosition, parent->FromDIP(wxSize(CARD_WIDTH, compact ? 66 : 82)))
        , m_small(compact)
    {
#ifdef __WXMSW__
        ...
        m_edit_btn = new ScalableButton(this, wxID_ANY, "edit", wxEmptyString, wxDefaultSize, wxDefaultPosition,
                                         wxBU_EXACTFIT | wxNO_BORDER, true, compact ? 12 : 14);
        ...
        if (compact) {
            m_remove_btn = new ScalableButton(this, wxID_ANY, "cross", wxEmptyString, wxDefaultSize, wxDefaultPosition,
                                               wxBU_EXACTFIT | wxNO_BORDER, true, 12);
            m_remove_btn->SetToolTip(_L("Remove filament"));
            m_remove_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) { if (m_on_remove) m_on_remove(); });
            overlay->Add(m_remove_btn, 0, wxTOP | wxRIGHT, FromDIP(2));
        }
    ...
    // top_label is the "T%d" slot-0 label ("" for swap cards). type_line/vendor_line are the
                      ...
                      const wxString& vendor_line, bool edit_disabled, const wxString& disabled_reason = wxString())
    {
        ...
        if (m_remove_btn)
            m_remove_btn->SetBackgroundColour(color);
        Refresh();
    }

    void set_on_edit(std::function<void()> cb) { m_on_edit = std::move(cb); }
    void set_on_remove(std::function<void()> cb) { m_on_remove = std::move(cb); }
...
private:
    ...
    void doRender(wxDC& dc)
    {
        ...
        if (m_remove_btn)
            icon_cluster_width += m_remove_btn->GetSize().GetWidth() + FromDIP(2);
        const int max_text_width = std::max(0, size.x - FromDIP(6) - FromDIP(4) - icon_cluster_width);

        int y = FromDIP(m_small ? 6 : 8);
    ...
    ScalableButton*        m_remove_btn{nullptr}; // small (swap) cards only
    ...
    , m_printer_preset_name(printer_preset_name)
{
    ...
    if (printer_preset) {
        if (const auto* opt = printer_preset->config.option<ConfigOptionBool>("enable_manual_filament_swap"))
            m_allow_manual_swaps = opt->value;
    }
    ...
    // Horizontal card strip: one column per tool, each column stacking that tool's main (loaded)
    // card on top, its swappable-filament cards below, and (when allowed) an "Add filament"
    // affordance under those.
    ...
    for (size_t i = 0; i < tool_count; ++i) {
        ...
        group.main_card = new FilamentCard(this, /*compact=*/false);
        col_sizer->Add(group.main_card, 0, wxALIGN_CENTER_HORIZONTAL);
        col_sizer->AddSpacer(FromDIP(6));

        group.swap_sizer = new wxBoxSizer(wxVERTICAL);
        col_sizer->Add(group.swap_sizer, 0, wxALIGN_CENTER_HORIZONTAL | wxEXPAND);

        // A fixed 1:1 tool->filament loadout is the default; offering
        // to record extra manually-swapped filaments is gated on the printer's opt-in flag. Rows
        // already recorded as swappable are still shown (and removable) either way -- they're
        // real machine state, the flag only controls adding more.
        if (m_allow_manual_swaps) {
            ...
            col_sizer->AddSpacer(FromDIP(6));
            col_sizer->Add(add_btn, 0, wxALIGN_CENTER_HORIZONTAL);
        }
    ...
    // Seeds every tool group's rows from the current device and paints their cards -- the same
    // path add_filament()/remove_filament()/device-switch use, so initial layout and post-edit
    // layout share one code path.
...
void FilamentInventoryEditor::update_card(size_t tool_idx, size_t row_idx, FilamentCard* card)
{
    ...
    const bool edit_disabled = tag_locked || printer_empty;
    ...
    if (!is_loaded)
        card->set_on_remove([this, tool_idx, row_idx]() { remove_filament(tool_idx, row_idx); });
}
...
void FilamentInventoryEditor::rebuild_tool_rows(size_t tool_idx)
{
    ...
    group.swap_sizer->Clear(true);
    group.swap_cards.clear();
    for (size_t r = 1; r < group.rows.size(); ++r) {
        auto* card = new FilamentCard(this, /*compact=*/true);
        group.swap_cards.push_back(card);
        group.swap_sizer->Add(card, 0, wxBOTTOM, FromDIP(6));
        update_card(tool_idx, r, card);
    }
