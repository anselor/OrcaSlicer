...

    {
        ...
        if (physical_printer_config != nullptr) {
            if (device_owned_mapping_protocol(*physical_printer_config)) {
                const int resolved_for_map = plate_idx == PLATE_ALL_IDX ? get_partplate_list().get_curr_plate_index() : resolved_plate_idx;
                PartPlate* part_plate = get_partplate_list().get_plate(resolved_for_map);
